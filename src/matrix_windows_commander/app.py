...
class MainWindow(QMainWindow):
    def __init__(self, language: str | None = None) -> None:
        ...
        self.setWindowTitle(f"{tr(self.language, 'app')} {__version__}")
    ...
    def _apply_icon(self) -> None:
        for name in ("matrix-windows-commander.ico", "matrix-windows-commander.svg"):
    ...
    def _build(self) -> None:
        ...
        header = QHBoxLayout()
        title = QLabel(tr(self.language, "app"))
        ...
        self.lang = QComboBox()
        self.lang.addItems(["PL", "EN"])
        self.lang.setCurrentText("PL" if self.language == "pl" else "EN")
        self.lang.currentTextChanged.connect(self._change_language)
        header.addWidget(self.lang)
        ...
        self.category.addItem(tr(self.language, "all"), "All")
        for item in CATEGORIES:
            self.category.addItem(item, item)
        self.category.currentIndexChanged.connect(self._populate)
        left_l.addWidget(self.category)
        self.commands = QListWidget()
        self.commands.currentItemChanged.connect(self._select)
        left_l.addWidget(self.commands, 1)
        splitter.addWidget(left)

        middle = self._panel()
        mid = QVBoxLayout(middle)
        self.detail_title = QLabel(tr(self.language, "details"))
        self.detail_title.setStyleSheet("font-size:15pt;font-weight:700;color:#7bdcff")
        mid.addWidget(self.detail_title)
        self.description = QPlainTextEdit()
        self.description.setReadOnly(True)
        mid.addWidget(self.description, 1)
        self.command_preview = QLineEdit()
        self.command_preview.setReadOnly(True)
        mid.addWidget(self.command_preview)
        row = QHBoxLayout()
        self.run_btn = QPushButton(tr(self.language, "run"))
        self.run_btn.setObjectName("Primary")
        self.run_btn.clicked.connect(self._run)
        row.addWidget(self.run_btn)
        self.copy_btn = QPushButton(tr(self.language, "copy"))
        self.copy_btn.clicked.connect(self._copy)
        row.addWidget(self.copy_btn)
        self.admin_btn = QPushButton(tr(self.language, "admin"))
        self.admin_btn.clicked.connect(self._elevate)
        row.addWidget(self.admin_btn)
        mid.addLayout(row)
        self.legacy_note = QLabel(tr(self.language, "legacy_note"))
        self.legacy_note.setWordWrap(True)
        self.legacy_note.setStyleSheet("color:#6f94ad;font-size:9pt")
        mid.addWidget(self.legacy_note)
        splitter.addWidget(middle)

        right = self._panel()
        right_l = QVBoxLayout(right)
        out_title = QLabel(tr(self.language, "output"))
        out_title.setStyleSheet("font-size:15pt;font-weight:700;color:#7bdcff")
        right_l.addWidget(out_title)
        self.output = QPlainTextEdit()
        self.output.setReadOnly(True)
        self.output.setLineWrapMode(QPlainTextEdit.NoWrap)
        right_l.addWidget(self.output, 1)
        out_buttons = QHBoxLayout()
        self.stop_btn = QPushButton(tr(self.language, "stop"))
        self.stop_btn.setObjectName("Danger")
        self.stop_btn.setEnabled(False)
        self.stop_btn.clicked.connect(self._stop)
        out_buttons.addWidget(self.stop_btn)
        clear = QPushButton(tr(self.language, "clear"))
        clear.clicked.connect(self.output.clear)
        out_buttons.addWidget(clear)
        right_l.addLayout(out_buttons)
        splitter.addWidget(right)
        splitter.setSizes([270, 430, 520])
        outer.addWidget(splitter, 1)

        footer = QHBoxLayout()
        self.footer = QLabel(tr(self.language, "footer"))
        self.footer.setCursor(Qt.PointingHandCursor)
        self.footer.mousePressEvent = lambda _e: webbrowser.open("https://github.com/Swir")
    ...
    def _populate(self, *_args) -> None:
        ...
        self.commands.clear()
        category = self.category.currentData() if hasattr(self, "category") else "All"
        ...
        for spec in values:
            ...
            item.setToolTip(spec.command_line)
            self.commands.addItem(item)
        if not values:
            self.commands.addItem(tr(self.language, "empty"))
            self.commands.item(0).setFlags(Qt.NoItemFlags)
        elif selected_id:
            for index in range(self.commands.count()):
                if self.commands.item(index).data(Qt.UserRole) == selected_id:
                    self.commands.setCurrentRow(index)
                    break
    ...
    def _select(self, current: QListWidgetItem | None, _previous=None) -> None:
        ...
        if not self.current:
            ...
            return
        spec = self.current
        desc = spec.description_pl if self.language == "pl" else spec.description_en
        risk_label = tr(self.language, spec.risk.value)
        admin = tr(self.language, "yes") if spec.admin_required else tr(self.language, "no")
        self.description.setPlainText(
            f"{desc}\n\n{tr(self.language, 'category')}: {spec.category}\n"
            f"{tr(self.language, 'risk')}: {risk_label}\n"
            f"{tr(self.language, 'requires_admin')}: {admin}\n"
            f"Timeout: {spec.timeout_seconds}s"
        )
        self.command_preview.setText(spec.command_line)
        self._sync_buttons()

    def _sync_buttons(self) -> None:
        running = self.process is not None and self.process.state() != QProcess.NotRunning if self.process else False
        can_run = bool(self.current) and not running and is_windows()
        self.run_btn.setEnabled(can_run)
        self.copy_btn.setEnabled(bool(self.current))
        self.stop_btn.setEnabled(running)
        self.admin_btn.setVisible(is_windows() and not is_admin())

    def _run(self) -> None:
        spec = self.current
        if not spec or not is_windows():
            return
        if spec.admin_required and not is_admin():
            QMessageBox.warning(self, tr(self.language, "app"), tr(self.language, "need_admin"))
            return
        if spec.risk in (Risk.REPAIR, Risk.CHANGE):
            ...
            if QMessageBox.question(
                self,
                tr(self.language, "confirm_title"),
                text,
        ...
        self.output.clear()
        self.output.appendPlainText(f"> {spec.command_line}\n")
        self.process = QProcess(self)
        self.process.setProgram(spec.executable)
        self.process.setArguments(list(spec.args))
        ...
        self.process.start()
        self.timeout.start(spec.timeout_seconds * 1000)
        self.statusBar().showMessage(f"{tr(self.language, 'running')}: {spec.title}")
        self._sync_buttons()
    ...
    def _stdout(self) -> None:
        if self.process:
            ...
            self.output.insertPlainText(self._decode(bytes(self.process.readAllStandardOutput())))

    def _stderr(self) -> None:
        if self.process:
            self.output.moveCursor(QTextCursor.End)
            self.output.insertPlainText(self._decode(bytes(self.process.readAllStandardError())))
    ...
    def _finished(self, exit_code: int, _status) -> None:
        self.timeout.stop()
        spec = self.current
        duration = int((time.monotonic() - self.started_monotonic) * 1000) if self.started_monotonic else 0
        ...
        self.statusBar().showMessage(f"{tr(self.language, 'finished')}: {exit_code}")
        if self.process:
    ...
    def _copy(self) -> None:
        if self.current:
            QApplication.clipboard().setText(self.current.command_line)
            self.statusBar().showMessage(tr(self.language, "copied"), 3000)
...
def run() -> None:
    ...
    app.setApplicationName("Matrix Windows Commander")
    icon = resource_path("assets/matrix-windows-commander.ico")
    if icon.exists():
